...
template.attachObj = (function(gridId){
    ...
    options.saveAttachment = function (){ console.log("saveAttachment");
            ...
            var subject = options.subject;  console.log("subject = " + subject);
            var attachmentType =  options.attachmentType;  console.log("attachmentType = " + attachmentType);
            var objType = options.getTypeSysNameBySubject(options.subject); console.log("objType = " +objType);
            form.dsCall('[frontws2]', 'attachmentCreate', {
               // OBJTYPENAME: options.getTypeSysNameBySubject(options.subject),
                ...
                if (resp.data.Status === 'OK'){ console.log("сохранение успешно"); console.log("subject2 = " + subject);
                    options.uploadECM(subject, attachmentType);
                    //options.updateECM(subject, attachmentType);
    ...
    options.updateAttachment = function () {console.log("updateAttachment"); console.log("options.subject = "+ options.subject);
            var uploadFile = options.uploadFile;
            var uploadFileType = options.attachmentType;
            var subject = options.subject;  console.log("subject = " + subject);
            var attachmentType =  options.attachmentType;  console.log("attachmentType = " + attachmentType);
        //тут нужно начитать id старого документа
             form.dsCall('[frontws2]', 'attachmentGetListByParams', {
                  ...
                  ORDERBY: "CREATIONDATE DESC",
                   ...
                   if (resp.data.TOTALCOUNT != 0){ console.log("старый отчет найден"); console.log("subject2 = " + subject); console.log("options.subject = " + options.subject);
                        var uploadFileID = resp.data.Result[0].ATTACHMENTID;           console.log("attachmentType2 = " + attachmentType); console.log("options.attachmentType = " + options.attachmentType);
                        ...
                        form.dsCall('[frontws2]', 'attachmentModify', {
                               ...
                               if (resp.data.Status === 'OK'){ console.log("обновление успешно"); console.log("subject3 = " + subject); console.log("attachmentType3 = " + attachmentType);
                                    options.updateECM(subject, attachmentType);
                                    //template.parentForm.action("REFRESH");
                               }
                        }).catch(function (e) {
                   ...
                   else {  console.log("отчет НЕ найден");
    ...
    options.updateECM = function (subject, attachmentType) { console.log("update_ECM"); console.log("subject = " + subject); console.log("attachmentType = " + attachmentType);
        var uploadFile = options.uploadFile;
    ...
    options.save = function () {
        ...
        if(uploadFileType == 'UWOFristReport') {
            form.dsCall('[frontws2]', 'docProcessingGetListByParams', {
                DOCUMENTID: options.subject,
                STAGESYSNAME: "UWOFristReportAttached",
                ORDERBY: "STAGEDATE DESC",
            }).then(function (resp) { console.log(resp);
                if (resp.data.TOTALCOUNT != 0){ console.log("этап UWOFristReportAttached найден");
        ...
        if(uploadFileType == 'BranchSignReport') {
            form.dsCall('[frontws2]', 'docProcessingGetListByParams', {
                DOCUMENTID: options.subject,
                STAGESYSNAME: "BranchSignReportAttached",
                ORDERBY: "STAGEDATE DESC",
            }).then(function (resp) {
                if (resp.data.TOTALCOUNT != 0){  console.log("этап BranchSignReportAttached найден");
                    options.updateAttachment();
                   // options.updateECM();
                }
                else {
       ...
       /* var projectSysname = form.getCurrentProjectSysname();
        ...
        form.startModalPageFlowProcess(projectSysname+pfd, {
                ATTACHE_LIST: options.uploadFile,
                Document_type: options.attachmentType,
                DocumentName: uploadFile.fileName,
			    STORAGE: uploadFile.storage,
                Loan_ID: options.subject,
			    fileInfoId: uploadFile.fileInfoId
        });*/
       /* template[gridId].hideEditor();
        template.attachBtn.enable();
